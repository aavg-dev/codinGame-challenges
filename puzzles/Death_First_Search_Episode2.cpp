...
//Change according to challenge
#define DEATH_FIRST_SEARCH_EPISODE_NUMBER 2 
...
public:
    ...
    void  SetEdgeCost(int from, int to, double NewCost) {
        ...
        //Copied Logic
        //TODO: The error seems to be here 
        if (!m_bDigraph)
        {
            for (curEdge = m_Edges[to].begin();
                curEdge != m_Edges[to].end();
...
private:
    void Search() 
    {
        ...
        cerr << "-------- SEARCH START -------" << endl;

        cerr << "Search starting, source " << m_iSource << " target " << m_iTarget << endl;
        ...
        while (!pq.empty())
        {
            ...
            cerr << "Next closest node is "<< NextClosestNode  << endl;
                ...
                pE = ConstEdgeItr.next())
            {
                ...
                {
                    cerr << "\tNew node "<<  pE->To() << " at frontier with cost " << pE->Cost() << endl;  
                    
                    m_CostToThisNode[pE->To()] = NewCost;

                    cerr << "\tNew cost to node " << pE->To() << " is " << NewCost << endl;
                ...
                {
                    cerr << "\tNode " << pE->To() << "previously analyzed, there is a shorter path with added cost " << pE->Cost() << endl;  
                    ...
                    cerr << "\tRe-visited cost to node " << pE->To() << ".New cost is " << NewCost << endl;

                    m_SearchFrontier[pE->To()] = pE;
                }
            }
            //cerr << "Cost to Node " << NextClosestNode << " is " <<  m_CostToThisNode[NextClosestNode] << endl;
        }
...
public:
    ...
    void InitializeGraph() {
        ...
        // Avega: This problem is a undirected graph
        // https://algs4.cs.princeton.edu/42digraph/
        // https://pediaa.com/what-is-the-difference-between-directed-and-undirected-graph/
        // The edges do NOT represent the direction
    ...
    void secureEdgeLink(int from, int to) {
        ...
        cerr << "Debug : new cost for edge is " << m_pGraph->GetEdge(from, to).Cost() << endl;
    }
...
Edge getSecuredEdge(const int episode_number,const std::list<int> gatewayPath){

    cerr << "Getting secured edge for episode " << episode_number << endl;
    cerr << "gateway path is ";
    for(auto it = gatewayPath.begin(); it != gatewayPath.end(); it++){
        cerr << *it << " " ;
    }

    std::vector<int> path(gatewayPath.begin(), gatewayPath.end());

    cerr << endl;
    if(episode_number == 1) {
        Edge securedEdge(path[0], path[1]);
        cerr << "Edge to secure is " << to_string(securedEdge) << endl;
        return securedEdge;
    } else if ( episode_number == 2){

        Edge securedEdge(path[path.size()-1], path[path.size()-2]);
        cerr << "Edge to secure is " << to_string(securedEdge) << endl;
        return securedEdge;
    }
    return Edge(0,0);
...
int main()
{
    ...
    while (1) {
        ...
        for(auto node : gateways)
        {
            ...
            g_Pathfinder->CreatePathDijkstra(); //Search is done when Path is created
        ...
        //Secure link between the closest node and the selected 
        Edge securedEdge = getSecuredEdge(DEATH_FIRST_SEARCH_EPISODE_NUMBER,gatewayPath);
        ...
        // Looks like we are returning directly without calculating the cost to the target node, check
